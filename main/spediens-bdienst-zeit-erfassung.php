...
<?php
switch ($retpath) {
    case "op": $rettarget = "op-doku.php?sid=$sid";
        break;
    case "spec": $rettarget = "spediens.php?sid=$sid";
        break;
    default: $rettarget = "op-doku.php?sid=$sid";
        break;
}
?>
    ...
    function closeifok()
    {
        ...
        {
            if (confirm("<?php echo $LDAlertNotSavedYet ?>")) {
...
//			alert("Zeitangabe ist ung�ltig! (ausserhalb des 24H Zeitrahmens)");
    ...
    function isgdatum(val, idx)
    {
        ...
        if ((xval3.length > 6) && (xval3.slice(xval3.length - 1, xval3.length) == "."))
            xval3 = xval3.slice(0, xval3.length - 1);
        if (xval3.length > 10)
            xval3 = xval3.slice(0, 10);
        xdoc.elements[idx].value = xval3;
...
';
    ...
    <table cellpadding="0" cellspacing=5 >
        <tr>
            <td>
<?php echo $LDStandbyPerson ?>:  <input type=text name="a_enc" size=30 value="<?php if (isset($a_enc)) echo $a_enc;
else echo $_COOKIE['ck_login_username']; ?>">
            </td>
            <td>
        ...
        <tr>
            ...
            <td align="right">
                ...
                -->
                <img <?php echo createLDImgSrc($root_path, 'printout.gif', '0', 'absmiddle') ?> ONCLICK="if (window.print) {
                            window.print();
                        } else {
                            window.alert('<?php echo $LDAlertNoechoer ?>');
                        }">
