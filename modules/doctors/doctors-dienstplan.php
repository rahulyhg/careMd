...
switch ($retpath) {
    case "menu": $rettarget = 'doctors.php' . URL_APPEND;
        break;
    case "qview": $rettarget = 'doctors-dienst-schnellsicht.php' . URL_APPEND . '&hilitedept=' . $dept_nr;
        break;
...
$smarty->assign('LDOnCall3', $LDDoc3);
...
for ($i = 1, $n = 0, $wd = $firstday; $i <= $maxdays; $i++, $n++, $wd++) {
    ...
    $selems = unserialize($dutyplan['duty_3_txt']);
    $a_pnr = unserialize($dutyplan['duty_1_pnr']);
    $r_pnr = unserialize($dutyplan['duty_2_pnr']);
    $s_pnr = unserialize($dutyplan['duty_3_pnr']);
    ...
    echo '&nbsp;<a href="javascript:popinfo(\'' . $r_pnr['hr' . $n] . '\')">' . $relems['r' . $n] . '</a>
        ...
        <td height=5 ' . $backcolor . '>';
    echo '&nbsp;<a href="javascript:popinfo(\'' . $s_pnr['hs' . $n] . '\')">' . $selems['s' . $n] . '</a>
	</td>
	</tr>';
    if ($wd == 6)
        $wd = -1;
