...
for ($i = 0; $i < count($item_array); $i++) {
    ...
    ?>
            ...
            if ($GLOBAL_CONFIG['transmit_to_weberp_enabled']=='1' && $notify_doctor_status=='1' && $class == 'drug_list' || $class == 'special_others_list' || $class == 'special_ctc_list' || $class == 'drug_list_ctc' || $class == 'drug_list_nhif' ) { //start of purchasing class
                         # code...
                    ...
                    //Check stock location in weberp and care2x
                     ...
                     // print_r($item);
                     if (!empty($care2x_pharmacy)) {//Pharmacy tagged to this patient
                         
                          // echo 'c2x partcode'.$partcode_c2x.'<br>';
                          // echo 'weberp stockid'.$item['stockid'];

                        if ($partcode_c2x==$item['stockid']) {//item match
                            $Bal=$weberp_obj->get_stock_balance_webERP($partcode);
