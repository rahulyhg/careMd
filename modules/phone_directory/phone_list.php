...
if (empty($displaysize) || !is_numeric($displaysize))
    $displaysize = 10;
    ...
    <table  border=0 cellpadding=0 cellspacing=0 width="100%">
        <tr>
            <td colspan=3><nobr>
                <?php
                if (!$edit) {
                    echo '<a href="phone.php' . URL_APPEND . '"><img ' . createLDImgSrc($root_path, 'such-gray.gif', '0') . '';
                    if ($cfg['dhtml'])
                        echo' style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)';
                    echo '></a>';
                }
                ?><img <?php echo createLDImgSrc($root_path, 'phonedir-b.gif', '0') ?>><?php
                if (!$edit)
                    echo '<a href="phone_edit_pass.php' . URL_APPEND . '">';
                else
                    echo "<a href=\"phone_edit.php" . URL_APPEND . "&remark=fromlist&sid=$sid&edit=$edit\">";
        ...
        if ($rows) {
            ...
            if (!$batchnum) {
                ...
                if (($linecount % $displaysize) > 0)
                    $pagecount = ($linecount / $displaysize) + 1;
                else
                    $pagecount = ($linecount / $displaysize);
            ...
            if ($update)
                echo "<font color=maroon size=3>$LDUpdateOk</font>";
            ...
            if ($pagecount > 1) {
                ...
                for ($i = 1; $i <= $pagecount; $i++) {
                    if ($i == $batchnum)
                        ...
                        echo ' <a href=phone_list.php' . URL_APPEND . '&pagecount=' . $pagecount . '&linecount=' . $linecount . '&batchnum=' . $i . '&displaysize=' . $displaysize . '&edit=' . $edit . '>' . $i . '</a> - ';
                }

                if ($batchnum < $pagecount)
                    echo '<a href=phone_list.php' . URL_APPEND . '&pagecount=' . $pagecount . '&linecount=' . $linecount . '&batchnum=' . ($batchnum + 1) . '&displaysize=' . $displaysize . '&edit=' . $edit . '><font color=red ><img src="../../gui/img/common/default/bul_arrowgrnlrg.gif" border=0></font></a>';
                echo "</FONT>\n";
            }
