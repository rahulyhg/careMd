...
if ($user_origin == 'pers' && $nr) {
    if (!$employee->loadPersonellData($nr))
        $mode = 'save';
    elseif ($employee->PhoneKey())
        $mode = 'update';
}
?>
    ...
    <td class="wardlisttitlerow" colspan=3 >
        &nbsp;<b>
            ...
            if (($newvalues == '') && ($remark != 'fromlist')) {
                $nowtime = date(G);
                if (($nowtime >= 0) && ($nowtime < 10))
                    echo $LDGoodMorning;
                elseif (($nowtime > 9) && ($curtime < 18))
                    echo $LDGoodDay;
                elseif ($nowtime > 18)
                    echo $LDGoodEvening;
                    ...
                    <tr>
                        <td colspan=3>
                            <p>
                                <input type="hidden" name="sid" value="<?php echo $sid; ?>">
                                <input type="hidden" name="lang" value="<?php echo $lang; ?>">
                                <input type="hidden" name="mode" value="<?php if (!empty($mode)) echo $mode;
                                else echo 'save'; ?>">
                                <input type="hidden" name="user_origin" value="<?php echo $user_origin ?>">
                                <input type="hidden" name="edit" value="<?php echo $edit ?>">
