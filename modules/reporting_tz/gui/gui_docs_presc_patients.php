...
//                                        $encounter_no = $data['patient_dets'][$i]['encounter_nr'];
                    $encounters = array();
                     ...
                     $sql_encounters="SELECT DISTINCT cen.encounter_nr
                        FROM  care_encounter_notes cen
                        WHERE 
                         date like '%$date%'  ";
                    

                    $result_encounters=$db->Execute($sql_encounters);

                    
                    while ( $row_encounter_no=$result_encounters->FetchRow()) {
                            $encounter_no=$row_encounter_no['encounter_nr'];
//                                            echo $encounter_no.' ll';
                        //get all the patient notes
                        $notes_qry = "SELECT MIN(nr), notes,personell_name FROM  care_encounter_notes INNER JOIN care_users ON care_users.name=care_encounter_notes.personell_name INNER JOIN care_user_roles ON care_user_roles.role_id=care_users.role_id "
                                               . " WHERE encounter_nr =$encounter_no "
                                               . " AND date = '$date' AND care_user_roles.Description='doctor'";
                                               $db_notes = $db->Execute($notes_qry);

                                           while ($row_notes = $db_notes->FetchRow()) {
                                                    if($row_notes['personell_name']==$doc_name){
...
//                                            echo $lab_qry . '<br>';
                        ...
                        $rows =  count($data['notes'][$encounter_no]);
...
//                                        . ' ' . count($data['lab']) . '  ' . count($data['notes']) . 'end' . $encounter_no . ' <br>';
                        ...
                        if ($rows < 1) {
                            ...
                            if(count($data['notes'][$encounter_no])>0){
