...
if (!$dept)
    if ($_COOKIE[ck_thispc_dept])
        ...
        $dept = 'plop'; // set to default plop = plastic surgery op
...
<ul>
    <FONT  class="prompt">
        ...
        if ($pmonth < 2)
            echo '12&pyear=' . ($pyear - 1) . '" title="' . $LDLastMonth . '">&lt;' . $monat[11];
        else
            echo ($pmonth - 1) . '&pyear=' . $pyear . '" title="' . $LDLastMonth . '">&lt;' . $monat[$pmonth - 2];
        echo '</a></td><td  align=center>';
        echo '<FONT color=navy><b>' . $monat[$pmonth - 1] . ' ' . $pyear . '</b></font>';
        echo '</td><td align=right>';
        echo '<a href="nursing-station-archiv.php' . URL_APPEND . '&pmonth=';
        if ($pmonth > 11)
            echo '1&pyear=' . ($pyear + 1) . '" title="' . $LDNextMonth . '">' . $monat[0];
        else
            echo ($pmonth + 1) . '&pyear=' . $pyear . '" title="' . $LDNextMonth . '">' . $monat[$pmonth];
        ...
        for ($x = 0; $x < 6; $x++) {
            ...
            for ($n = 0; $n < 6; $n++) {
                if ($daynumber[$j] . $pmonth . $pyear == date(jnY))
                    echo '
			  <td bgcolor=orange>';
                else
                    echo '
	  ...
	  <FONT  face="times new roman"   SIZE=5 ';
            ...
            echo '><b>&nbsp;';
            if ($pyear . $mn . $dn <= date(Ymd))
                echo '<a href="nursing-schnellsicht.php' . URL_APPEND . '&from=arch&dept=' . $dept . '&edit=0&pday=' . $dn . '&pmonth=' . $mn . '&pyear=' . $pyear . '">';
            echo $daynumber[$j];
            if ($pyear . $mn . $dn <= date(Ymd))
                echo '</a>';
            echo '</b></td>';
...
<a href="#" onClick=cxjahr(\'0\')><img ' . createComIcon($root_path, 'varrow-d.gif', '0') . ' alt="' . $LDMinusYear . '"></a>';
    ...
    echo'<input  type="button" value="+1" onClick=cxjahr(\'1\')> <input  type="button" value="-1" onClick=cxjahr(\'0\')>';
?>
