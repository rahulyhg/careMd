...
if ($mode == 'save') {
    ...
    for ($i = 0; $i < $maxelement; $i++) {
        ...
        //echo $$ddx.$$tdx;
        if ($charts_obj->saveBPFromArray($data_array))
            $saved = 1;
    }
    ...
    for ($i = 0; $i < $maxelement; $i++) {
        ...
        $data_array['msr_time'] = strtr($$tdx, ':,;-/_', '......');
        $data_array['value'] = $$ddx;
        if ($charts_obj->saveTemperatureFromArray($data_array))
            $saved = 1;
    }
...
<BODY  bgcolor="#99ccff" TEXT="#000000" LINK="#0000FF" VLINK="#800080"   topmargin="0" marginheight="0" 
       onLoad="<?php if ($saved) echo "parentrefresh();"; ?>if(window.focus) window.focus();
               window.focus();" >
    ...
    ?>
                                    <table border=0 cellpadding=1 cellspacing=1 width=100%>
                                        ...
                                        for ($i = 0; $i < $rcount; $i++) {
                                            ...
                                            <?php
                                            if (!empty($bb['msr_time']))
                                                echo $bb['msr_time'];
                                            ?>
                                                </td>
                                                <td><FONT SIZE=-1  FACE="Arial"><?php if ($bb['value']) echo $bb['value'];
                                            else echo '&nbsp;'; ?></td>
                                            </tr>
