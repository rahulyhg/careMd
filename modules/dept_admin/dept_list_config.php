          ...
          <!-- 	<td bgcolor="#e9e9e9"><img <?php echo createComIcon($root_path, 'arrow_blueW.gif', '0'); ?>></td>
                ...
                <td class=pblock  bgColor="#eeeeee"><a href="dept_info.php<?php echo URL_APPEND . "&dept_nr=" . $v['nr']; ?>">
                        ...
                        if (isset($$v['LD_var']) && $$v['LD_var'])
                            ...
                            echo $v['name_formal'];
                        ?></a> </td>
                <td class=pblock  bgColor="#eeeeee"><?php if ($v['is_inactive']) echo '<font color="red">' . $LDInactive . '</font>';
                        else echo $LDActive ?> 
                    [<a href="dept_status.php<?php echo URL_APPEND; ?>&nr=<?PHP echo $v['nr'] . "&active=" . $v['is_inactive']; ?>">x</a>]
                </td>
                <td class=pblock  bgColor="#eeeeee"><?php if ($v['status'] == 'hidden') echo '<font color="red">' . $LDHidden . '</font>';
                        else echo $LDVisible ?> 
                    [<a href="dept_status.php<?php echo URL_APPEND; ?>&nr=<?PHP echo $v['nr']; ?>&status=<?PHP if ($v['status'] == 'hidden') echo 'visible';
        else echo "hidden"; ?>">x</a>]
                </td>
