...
if (isset($mode) && ($mode == 'search' || $mode == 'paginate') && isset($searchkey) && ($searchkey)) {
    ...
    if (is_numeric($suchwort)) {
        ...
        if (sizeof($comp) > 1) {
            ...
            if (empty($odir))
                $odir = 'DESC';# default, latest birth at top
        }else {
            ...
            if ($DOB)
                ...
                $sql2.=')';
            if (empty($odir))
                $odir = 'ASC';# default, ascending alphabetic
        }
    }
    ...
    $sql = "SELECT ps.nr, ps.is_discharged, UPPER(p.name_last) as name_last, CONCAT(p.name_first,' ', p.name_2) AS name_first, p.date_birth, p.addr_zip, p.sex,p.photo_filename " . $dbtable . $sql2 . $sql3;
    //echo $sql;
