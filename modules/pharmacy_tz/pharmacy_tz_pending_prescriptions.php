...
if($transmit_to_weberp_enabled==="1"){
          ...
          WHERE p.pid=" . $_SESSION['sess_pid'] . " AND p.pid=e.pid AND e.encounter_nr=pr.encounter_nr 
                      ...
                      AND ( purchasing_class = 'drug_list' OR purchasing_class ='drug_list_ctc' OR purchasing_class='drug_list_nhif' OR purchasing_class='supplies' OR purchasing_class='dental') AND 
          ...
          //echo $sql;
            ...
            $sql = "SELECT pr.*, e.encounter_class_nr FROM care_encounter AS e, care_person AS p, care_encounter_prescription AS pr, care_tz_drugsandservices
          WHERE p.pid=" . $_SESSION['sess_pid'] . " AND p.pid=e.pid AND e.encounter_nr=pr.encounter_nr 
                      AND pr.article_item_number=care_tz_drugsandservices.item_id AND ( purchasing_class = 'drug_list' OR purchasing_class ='special_others_list' 
                      OR purchasing_class ='drug_list_ctc' OR purchasing_class='drug_list_nhif' OR purchasing_class='dental' OR purchasing_class='supplies') AND pr.encounter_nr=".$pn."  AND (pr.status='pending' OR pr.status='') AND pr.prescribe_date='$prescription_date'
          ORDER BY pr.modify_time DESC";
        }
