...
if (isset($_GET['save'])) {
    ...
    if (($insurance_ID != '') && !$insuranceParentSame && (!$wrong_max_pay)) { //update existing insurance
        $values_old = array();
        foreach ($variables as $val)
    ...
    } else if (!$insuranceParentSame) {   //create a new insurance
...
else if ((($insurance_ID == '') || isset($toggle)) && $show) { // when you first enter the menu:
