...
class Bill extends Encounter {
    ...
    function getEncounterFromBillNo($billNo) {
        ...
        $this->sql = "SELECT encounter_nr FROM $this->tbl_bill WHERE nr=$billNo";
...
//		echo $this->sql;
    ...
    function StorePrescriptionItemToBill($pid, $prescriptions_nr, $bill_number, $insurance, $pricelist_no,$bank) {
       ...
       // echo $this->sql;
		...
		INSERT INTO $this->tbl_bill_elements " .
                ...
                "		User_Id)" .
                "
		VALUES (" . $bill_number . ", '" . time() . "', 0, 1, 0, 0, 0, '" . $row['total_dosage'] . "', '" . $row['total_dosage'] . "', " . $row['times_per_day'] . "," . $row['days'] . ", " . $price . ", '".$row['materialcost']."', '" . $row['article'] . "', '" . $row['notes'] . "', " . $row['article_item_number'] . ", '" . $insurance . "', '" . $contract['id'] . "', '" . $row['nr'] . "','" . $row['sub_store'] . "','".$bank."','" . $row['current_dept_nr'] . "','" . $row['current_ward_nr'] . "','" . $row['encounter_class_nr'] . "','" . $user_id . "')";
    ...
    function StoreLaboratoryItemToBill($pid, $labtest_nr, $batch_nr, $bill_number, $insurance, $pricelist_no,$bank) {
        ...
        while ($row = $result->FetchRow()) {
			...
			INSERT INTO $this->tbl_bill_elements  " .
                    ...
                    "		User_Id)" .
                    "
			VALUES (" . $bill_number . ", '" . time() . "', 1, 0, 0, 0, 1, 1, 1, 0, 0, " . $price . ", '" . $row['item_description'] . "', 0, " . $row['p_item_id'] . ", '" . $insurance . "', '" . $contract['id'] . "', '" . $row['prescrip_nr'] . "','".$bank."', '" . $row['current_dept_nr'] . "', '" . $row['current_ward_nr'] . "', '" . $row['encounter_class_nr'] . "','" . $user_id . "')";
 ...
 '" . $row['item_description'] . "', " . $row['item_id'] . ", '" . $insurance . "', '" . $contract['id'] . "', " . $row['batch_nr'] . ",'".$bank."'," . $row['current_dept_nr'] . ", " . $row['current_ward_nr'] . ", " . $row['encounter_class_nr'] . ",'" . $user_id . "')";
...
//------------------------------------------------------------------------------
    ...
    function DisplayAdvanceForRefund($bill_nr,$edit_fields, $printout){
        ...
        while ($bill_elems_row = $billelems->FetchRow()) {
            ...
            $this->sql="SELECT * FROM care_tz_billing_elem WHERE nr=".$bill_nr;
            $this->result=$db->Execute($this->sql);
            ...
            while ($bills = $billnumbers->FetchRow()) {
                ...
                echo '
										...
										<tr>
											...
											<td><b><font color="#FF0000"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="2">
...
//                $sum_to_pay += $total_advance;
                ...
                if ($balance != 0) {    //Only show when balance is not zero
                    if ($balance < 0) {
                        echo '<tr>
    ...
    function ArchiveBill($bill_number) {
        ...
        $this->sql = "INSERT INTO care_tz_billing_archive_elem
                                    (`nr`,`date_change`, `is_labtest`,`is_radio_test`, `is_medicine`, `is_comment`, `is_paid`, `amount`, `days`, `times_per_day`, `price`,`materialcost`, `balanced_insurance`, `insurance_id`, `description`,`notes`, `item_number`, `prescriptions_nr`,`bank_ref`,`sub_store`,`is_deposit_item`, `User_Id`,current_dept_nr,current_ward_nr,encounter_class_nr)
                                    SELECT `nr`,`date_change`, `is_labtest`,`is_radio_test`, `is_medicine`, `is_comment`, 1, `amount`, `days`, `times_per_day`, `price`,`materialcost`, `balanced_insurance`, `insurance_id`, `description`, `notes`,`item_number`, `prescriptions_nr`,`bank_ref`,`sub_store`,`is_deposit_item`,`User_Id`,current_dept_nr,current_ward_nr,encounter_class_nr FROM care_tz_billing_elem WHERE `nr`=" . $bill_number;
        $this->result = $db->Execute($this->sql);
