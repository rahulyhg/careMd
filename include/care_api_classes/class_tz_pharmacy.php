...
class Product extends Core {
  ...
  function get_array_search_results($keyword, $result_filter){
    ...
    if($keyword=="*") {
	    $this->sql="SELECT `item_id`, `partcode`, 100 as plausibility, `item_description`,`not_in_use`,`min_level` FROM ".$this->tbl_product_items." WHERE ".$SQL_FILTER;
	    return $db->Execute($this->sql);

  	} else {
	  	$this->sql="SELECT `item_id`, `partcode`, `item_description`,`not_in_use`,`min_level` FROM ".$this->tbl_product_items." WHERE `purchasing_class`='".$keyword."'";
