...
function getDateFormat() {
    ...
    if ($errFormat) {
        ...
        if ($df['date_format'] != '')
            ...
            return 'dd.MM.yyyy'; // this is the last alternative format (german traditional)
    }
}
...
function formatDate2Local($stdDate, $localFormat, $retTime = FALSE, $timeOnly = FALSE, $sepChars = '') {
    ...
    if (preg_match('/0000/i', $stdDate))
        return strtr($localFormat, 'yYmMdDHis', '000000000'); // IF  std date is 0 return 0's in local format

        /* If time is included then isolate */
    if (strchr($stdDate, ':')) {
        list($stdDate, $stdTime) = explode(' ', $stdDate);
        if ($timeOnly)
            return $stdTime; /* If time only is needed */
    }
    ...
    for ($i = 0; $i < 3; $i++) {
        if ($localArray[$i] == 'yyyy')
            $localArray[$i] = $stdArray[0];
        elseif ($localArray[$i] == 'mm')
            $localArray[$i] = $stdArray[1];
        elseif ($localArray[$i] == 'dd')
            $localArray[$i] = $stdArray[2];
    }
